...
int ul_fec_decode(ul_fec_decoder_t *decoder, uint8_t *output) {
    ...
    // Simple reconstruction using XOR (placeholder)
    // Real Reed-Solomon would solve linear equations over GF(256)
    
    size_t total_size = 0;
    uint8_t shards_found = 0;
    
    // Copy available data shards
    for (uint8_t i = 0; i < decoder->params.data_shards; i++) {
        if (decoder->shard_present[i]) {
            memcpy(output + total_size, decoder->shards[i], decoder->params.shard_size);
            total_size += decoder->params.shard_size;
            shards_found++;
        }
    }
    
    if (shards_found < decoder->params.data_shards) {
...
int ul_delta_encode_gps(ul_delta_ctx_t *ctx, const ul_gps_raw_t *gps,
    ...
    if (!ctx->has_previous) {
        ...
        // Copy other fields (simplified - could also delta encode)
        memcpy(&output[pos], &gps->eph, 2); pos += 2;
        memcpy(&output[pos], &gps->vel, 2); pos += 2;
...
int ul_delta_encode_attitude(ul_delta_ctx_t *ctx, const ul_attitude_t *att,
    ...
    if (max_output < sizeof(ul_attitude_t)) return -1;
...
int ul_delta_decode_attitude(ul_delta_ctx_t *ctx, const uint8_t *delta_data,
    ...
    if (delta_len < sizeof(ul_attitude_t)) return -1;
...
int ul_delta_encode_battery(ul_delta_ctx_t *ctx, const ul_battery_t *bat,
                            uint8_t *output, size_t max_output) {
    if (max_output < sizeof(ul_battery_t)) return -1;
...
int ul_delta_decode_battery(ul_delta_ctx_t *ctx, const uint8_t *delta_data,
    ...
    if (delta_len < sizeof(ul_battery_t)) return -1;
...
int ul_pack_phase3(const ul_header_t *header, const uint8_t *payload, size_t payload_len,
    ...
    if (max_output < payload_len + 32) return -1;
...
int ul_parse_phase3(const uint8_t *input, size_t input_len,
    ...
    // Simplified parser
    if (input_len > max_payload) return -1;
