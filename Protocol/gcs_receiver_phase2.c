...
int main(void) {
    ...
    switch(caps->backend) {
        ...
        case UL_CRYPTO_ARM_NEON: printf("ARM NEON (%ux speedup)\n", caps->speedup_factor); break;
        case UL_CRYPTO_X86_SSE:  printf("x86 SSE (%ux speedup)\n", caps->speedup_factor); break;
        case UL_CRYPTO_X86_AVX2: printf("x86 AVX2 (%ux speedup)\n", caps->speedup_factor); break;
    ...
    printf("Success rate: %.2f%%\n",
           ...
           (100.0 * g_stats.parse_complete / g_stats.packets_received) : 0.0);
    if (g_stats.parse_complete > 0) {
