#ifndef KESTREL_FAST_H
#define KESTREL_FAST_H
 ...
 */
int ks_parse_char_zerocopy(ks_parser_zerocopy_t *parser, uint8_t byte, uint8_t *output_buf);

/* --- Memory Pool Allocator --- */

#define KS_MEMPOOL_NUM_BUFFERS 32  // Number of buffers in pool
#define KS_MEMPOOL_BUFFER_SIZE 512 // Size of each buffer (matches max packet)
...
typedef struct
{
    uint8_t buffers[KS_MEMPOOL_NUM_BUFFERS][KS_MEMPOOL_BUFFER_SIZE];
 ...
 * @param pool Memory pool
 * @param ptr  Pointer previously returned by ks_mempool_alloc()
 */
...
typedef enum
{
    KS_CRYPTO_SOFTWARE = 0, // Pure software (monocypher)
    KS_CRYPTO_ARM_NEON,     // ARM NEON SIMD instructions
    KS_CRYPTO_X86_SSE,      // x86 SSE2/SSE4
    KS_CRYPTO_X86_AVX2,     // x86 AVX2
    KS_CRYPTO_AES_NI        // x86 AES-NI hardware acceleration
} ks_crypto_backend_t;
...
typedef struct
{
    ks_crypto_backend_t backend; // Active crypto backend
 ...
 * @param buffer Output: pointer to allocated buffer (caller must free with ks_mempool_free)
...
int ks_pack_fast(ks_mempool_t *pool, const ks_header_t *h, const uint8_t *payload,
                 ...
                 ks_crypto_ctx_t *crypto_ctx, uint8_t **buffer);
