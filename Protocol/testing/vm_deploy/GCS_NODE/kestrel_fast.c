...
{
    memset(parser, 0, sizeof(ks_parser_zerocopy_t));
    ...
    case 4:                                                       // CRC (2 bytes)
        ...
        if (parser->bytes_received >= 2)
        {
            ...
            for (int i = 1; i < crc_header_len; i++)
                ks_crc_accumulate(parser->header_buf[i], &crc_calc);

            // Hash payload (ciphertext for encrypted, plaintext for unencrypted)
            for (int i = 0; i < parser->payload_len; i++)
                ks_crc_accumulate(parser->output_payload[i], &crc_calc);

            // For encrypted packets, also hash the 16-byte MAC tag
            if (crc_encrypted)
            {
                for (int i = 0; i < 16; i++)
                    ks_crc_accumulate(parser->cipher_tag[i], &crc_calc);
            }

            ks_crc_accumulate(ks_get_crc_seed(parser->msg_id), &crc_calc);
                 ...
                 ks_crypto_ctx_t *crypto_ctx, uint8_t **buffer)
{
    ...
    if (key_32b && crypto_ctx)
    {
        ...
        packed_len = kestrel_pack_cached(buf, h, payload, key_32b, nonce_state, crypto_ctx);
    }
    else if (key_32b)
    {
        // Use selective encryption
        packed_len = kestrel_pack_selective(buf, h, payload, key_32b, nonce_state);
    }
    else
    {
        // No encryption
        packed_len = kestrel_pack(buf, h, payload, NULL);
    }
...
int ks_parse_char_fast(ks_parser_zerocopy_t *parser, uint8_t byte, ks_mempool_t *pool)
{
    ...
    int result = ks_parse_char_zerocopy(parser, byte, parser->output_payload);
