                                       ...
                                       uint8_t *ciphertext, uint8_t mac[16])
{
    ...
    ks_chacha20_neon(key, nonce, poly_key, poly_key, 32);
...
#if KS_HW_AVX2_AVAILABLE
    ks_chacha20_avx2(key, nonce, input, output, len);
#elif KS_HW_NEON_AVAILABLE
    ks_chacha20_neon(key, nonce, input, output, len);
#elif KS_HW_SSE2_AVAILABLE
    ks_chacha20_sse2(key, nonce, input, output, len);
#else
