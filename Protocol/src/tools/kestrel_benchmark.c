/*
 * Kestrel Comprehensive Performance Profiler
 *
...
static ks_session_t TEST_SESSION;
...
static void init_test_session(void)
{
    ...
    if (f != NULL)
    {
        ...
        if (read == 32)
        {
            ...
            ks_session_init(&TEST_SESSION, key);
            return;
        }
    }
...
void benchmark_baseline(benchmark_result_t *result, int iterations)
{
    ...
    for (int i = 0; i < iterations; i++)
    {
        ...
        int packet_len = kestrel_pack_with_nonce(packet, &header, payload, &TEST_SESSION);
        ...
        {
            ks_parse_char(&parser, packet[j], TEST_SESSION.key);
        }
        end = get_time_us();
...
void benchmark_phase1(benchmark_result_t *result, int iterations)
{
    ...
    for (int i = 0; i < iterations; i++)
    {
        ...
        uint64_t start = get_time_us();
        int packet_len = kestrel_pack_cached(packet, &header, payload, &TEST_SESSION, &crypto_ctx);
        uint64_t end = get_time_us();
        result->pack_time_us += (end - start);
        result->bytes_packed += packet_len;
...
void benchmark_phase2(benchmark_result_t *result, int iterations)
{
    ...
    for (int i = 0; i < iterations; i++)
    {
        ...
        uint64_t start = get_time_us();
        int packet_len = ks_pack_fast(&pool, &header, payload, &TEST_SESSION, &crypto_ctx, &packet);
        uint64_t end = get_time_us();
        result->pack_time_us += (end - start);
        ...
        for (int j = 0; j < packet_len; j++)
        {
            ks_parse_char_zerocopy(&parser, packet[j], parse_buf, sizeof(parse_buf));
        }
        end = get_time_us();
        result->parse_time_us += (end - start);
...
void benchmark_phase3(benchmark_result_t *result, int iterations)
{
    ...
    for (int i = 0; i < iterations; i++)
    {
        ...
        // Time delta encoding
        uint64_t start = get_time_us();
        int encoded_len = ks_delta_encode_gps(&delta_ctx, &gps, encoded, sizeof(encoded));
        uint64_t end = get_time_us();
        result->pack_time_us += (end - start);
...
{
    print_separator();
    printf("Kestrel Performance Profiler\n");
    print_separator();
    printf("\n");
    ...
    const ks_crypto_caps_t *caps = ks_crypto_get_caps();
    printf("  Crypto backend: %s (%ux speedup)\n",
           ks_crypto_backend_name(), caps->speedup_factor);
