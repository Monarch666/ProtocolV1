/*
 * Kestrel Hardware Crypto Acceleration - Implementation
 *
...
// Alignment macro
#if defined(_MSC_VER)
#define KS_ALIGN(n) __declspec(align(n))
#else
...
void ks_chacha20_neon(const uint8_t key[32], const uint8_t nonce[8],
                      const uint8_t *input, uint8_t *output, size_t len,
                      uint32_t initial_counter)
{
    // ChaCha20 state initialization - ensure 16-byte alignment for NEON
    KS_ALIGN(16) uint32_t state[16];
...
void ks_chacha20_poly1305_encrypt_neon(const uint8_t key[32], const uint8_t nonce[8],
                                       ...
                                       uint8_t *ciphertext, uint8_t mac[16])
{
    ...
    ks_chacha20_neon(key, nonce, plaintext, ciphertext, plaintext_len, 1);
    ...
    ks_chacha20_neon(key, nonce, poly_key, poly_key, 32, 0); // Explicit counter=0
...
int ks_chacha20_poly1305_decrypt_neon(const uint8_t key[32], const uint8_t nonce[8],
                                      const uint8_t *ad, size_t ad_len,
                                      ...
                                      const uint8_t mac[16], uint8_t *plaintext)
{
    ...
    uint8_t poly_key[32] = {0};
    ks_chacha20_neon(key, nonce, poly_key, poly_key, 32, 0);
    ...
    ks_chacha20_neon(key, nonce, ciphertext, plaintext, ciphertext_len, 1);
    return 0;
}

#endif // KS_HW_NEON_AVAILABLE
...
void ks_chacha20_sse2(const uint8_t key[32], const uint8_t nonce[8],
                       const uint8_t *input, uint8_t *output, size_t len,
                       uint32_t initial_counter)
{
    KS_ALIGN(16) uint32_t state[16];
...
void ks_chacha20_avx2(const uint8_t key[32], const uint8_t nonce[8],
                       const uint8_t *input, uint8_t *output, size_t len,
                       uint32_t initial_counter)
{
    KS_ALIGN(32) uint32_t state[16]; // AVX2 ideally needs 32-byte alignment
    state[0] = 0x61707865; state[1] = 0x3320646e; state[2] = 0x79622d32; state[3] = 0x6b206574;
    memcpy(&state[4], key, 32);
    state[12] = initial_counter; state[13] = 0;
    memcpy(&state[14], nonce, 8);
...
void ks_chacha20_auto(const uint8_t key[32], const uint8_t nonce[8],
                      const uint8_t *input, uint8_t *output, size_t len)
{
...
#if KS_HW_AVX2_AVAILABLE
    ks_chacha20_avx2(key, nonce, input, output, len, 1);
#elif KS_HW_NEON_AVAILABLE
    ks_chacha20_neon(key, nonce, input, output, len, 1);
#elif KS_HW_SSE2_AVAILABLE
    ks_chacha20_sse2(key, nonce, input, output, len, 1);
#else
...
const char *ks_crypto_backend_name(void)
{
    if (!g_hw_crypto_enabled)
    {
...
uint32_t ks_crypto_benchmark_1kb(void)
{
    uint8_t key[32] = {0};
    uint8_t nonce[8] = {0};
...
// Timing
    ...
    ks_chacha20_auto(key, nonce, input, output, 1024);
...
int ks_enable_hardware_crypto(void)
{
#if KS_HW_NEON_AVAILABLE || KS_HW_SSE2_AVAILABLE || KS_HW_AVX2_AVAILABLE
...
void ks_disable_hardware_crypto(void)
{
    g_hw_crypto_enabled = false;
}

bool ks_is_hardware_crypto_enabled(void)
{
