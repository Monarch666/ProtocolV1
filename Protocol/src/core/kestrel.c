...
<<<<<<<< HEAD:Protocol/testing/vm_deploy/UAV_NODE/kestrel.c
    return 0; // Unknown message
========
    // Hash-based seed for unknown/custom messages instead of 0
    return (uint8_t)((msg_id * 31 + 7) & 0xFF);
>>>>>>>> 1f0c66a (perf(keymanager): persistent CSPRNG fd + atomic three-stage key rotation):Protocol/src/core/kestrel.c
}
...
<<<<<<<< HEAD:Protocol/testing/vm_deploy/UAV_NODE/kestrel.c
    ...
    case KS_ENCRYPT_ALWAYS:
        if (!key_32b)
            return KS_ERR_NO_KEY; /* Policy violation - key required */
